...
from utils.nf_sf_first_mcp_flow_audit import (
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="NF-SF Full-Sequence first-MCP oracle-flow audit."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def run_audit(args: argparse.Namespace) -> dict[str, Any]:
    ...
    common_inputs.update(
        {
            "audit_schema": FIRST_MCP_FLOW_AUDIT_SCHEMA,
    ...
    try:
        audit = run_first_mcp_flow_audit(
    ...
    tensor_path = tensors_dir / "audit_tensors.pt"
    atomic_torch_save(audit.tensors, tensor_path)

    comparisons_dir = args.output_dir / "comparisons"
    comparisons_dir.mkdir(parents=True, exist_ok=True)
    comparison_paths = {}
    for name, report in audit.comparisons.items():
        path = comparisons_dir / f"{name}.json"
        atomic_json_write(report, path)
        comparison_paths[name] = str(path.resolve())
    ...
    manifest["comparison_paths"] = comparison_paths
    manifest["video_outputs"] = {
        mode: {
            "path": str((args.output_dir / mode / "output.mp4").resolve()),
            "sha256": file_sha256(args.output_dir / mode / "output.mp4"),
        }
        for mode in (TEACHER_REFERENCE, PREDICTED_FIRST_MCP, ORACLE_FLOW_FIRST_MCP)
    }
    atomic_json_write(manifest, args.output_dir / "first_mcp_flow_audit.json")
    return manifest
        ...
        json.dumps(
            {
                ...
                "step0_decisive_metrics": manifest["step0_decisive_metrics"],
            },
            indent=2,
            ensure_ascii=False,
        ),
