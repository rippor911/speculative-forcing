...
class BaseModel(nn.Module):
    ...
    def _initialize_models(self, args, device):
        ...
        # Next-Forcing Multi-Chunk Prediction heads (opt-in). Created before the
        # trainer FSDP-wraps the generator, so they land inside the same FSDP root
        # and are picked up by generator.parameters() for the optimizer and EMA.
        self.mcp_num_modules = int(getattr(args, "mcp_num_modules", 0))
        if self.mcp_num_modules > 0:
            # The RNG must be forked to a rank-INDEPENDENT seed here. The trainer calls
            # set_seed(config.seed + global_rank) (trainer/distillation.py:45), so every
            # rank draws different random numbers, and fsdp_wrap passes
            # sync_module_states=False (utils/distributed.py:65) so FSDP will not
            # broadcast rank 0's weights. The backbone is immune because it is
            # load_state_dict-ed from ode_init.pt on every rank, but the MCP heads have
            # no checkpoint: without this fork each rank would build DIFFERENT heads and
            # the hybrid_full (HYBRID_SHARD) replica groups would silently diverge.
...
class SelfForcingModel(BaseModel):
    ...
    def _initialize_inference_pipeline(self):
        ...
        self.inference_pipeline = SelfForcingTrainingPipeline(
            ...
            memory_gap_blocks=getattr(self.args, "memory_gap_blocks", 0),
